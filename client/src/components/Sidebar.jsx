...
export default function Sidebar({
  ...
  const [displayedContent, setDisplayedContent] = useState("Directs");
  const [contentTitle, setContentTitle] = useState("Inbox");
  const [currentActiveId, setCurrentActiveId] = useState(allConvo[0]._id);
  const [activeBtn, setActiveBtn] = useState("inbox-btn");
  const [isDarkMode, setIsDarkMode] = useState(false);
  const [activeInbox, setActiveInbox] = useState("direct");
  const [searchInput, setSearchInput] = useState("");
  const sideOptionStyle = " p-3 rounded-full cursor-pointer ";
  const activeColor = "bg-blue-800";

  useEffect(() => {
    // console.log("the current displayed content: ", displayedContent);
  }, [displayedContent]);
  useEffect(() => {
    console.log("the current search input: ", searchInput);
  }, [searchInput]);
  ...
  return (
    <div className="flex bg-white shadow-md mr-0.3">
      <div className="flex flex-col pt-4 gap-5 px-3 bg-white shadow-md">
        ...
        <button
          ...
          className={
            ...
            sideOptionStyle +
            (activeBtn === "inbox-btn" ? activeColor : "bg-white")
          }
          id="inbox-btn"
        >
          <svg
            ...
            width="25"
            height="25"
            fill={activeBtn === "inbox-btn" ? "white" : "#53575e"}
          >
      ...
      <div className="flex flex-col p-5 pt-2 min-w-50 w-100">
        ...
        <div
          className={`flex ${
            displayedContent === "Directs" || displayedContent === "Groups"
              ? "block"
              : "hidden"
          }`}
        >
          ...
          <div
            onClick={() => {
              setDisplayedContent("Groups");
              setContentTitle("Inbox");
              setActiveBtn("groups-btn");
              setActiveInbox("group");
            }}
            className={`cursor-pointer flex-grow text-center p-2 shadow-md ${
